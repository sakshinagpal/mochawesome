...
function Mochawesome (runner, options) {
  ...
  runner.on('end', function () {
    ...
    var color = (allFailures.length > 0) ? koColor : okColor;
    var status = allPasses.length + '/' + (allPasses.length + allFailures.length);
    var svgObj = shields.svg(subject, status, color, style);
    ...
    try {
      if (!endCalled) {
        ...
        saveToFile(svgObj, config.reportSvgFile, function(){});
